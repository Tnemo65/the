...
public class Float2TidSetLTTreeAggregateMap extends AbstractFloat2ObjectSortedMap<TIdSet>
        implements
        java.io.Serializable,
        Cloneable, LTAggregateMap {
    /**
     ...
     */
    protected transient ObjectSortedSet<Float2ObjectMap.Entry<TIdSet>> entries;
    /**
     * Cached set of keys.
     */
    ...
    protected TIdSet cachedSet = null;
    protected float previousK = 0f;
     ...
     */
    public Float2TidSetLTTreeAggregateMap() {
        tree = null;
        count = 0;
    }
     ...
     */
    public Float2TidSetLTTreeAggregateMap(final Comparator<? super Float> c) {
        this();
        storedComparator = c;
        setActualComparator();
    }
     ...
     */
    public Float2TidSetLTTreeAggregateMap(final Map<? extends Float, ? extends TIdSet> m) {
        this();
        putAll(m);
    }
     ...
     */
    public Float2TidSetLTTreeAggregateMap(final SortedMap<Float, TIdSet> m) {
        this(m.comparator());
        putAll(m);
    }
     ...
     */
    public Float2TidSetLTTreeAggregateMap(final Float2ObjectMap<? extends TIdSet> m) {
        this();
        putAll(m);
    }
     ...
     */
    public Float2TidSetLTTreeAggregateMap(final Float2ObjectSortedMap<TIdSet> m) {
        this(m.comparator());
        putAll(m);
    }
     ...
     */
    public Float2TidSetLTTreeAggregateMap(final float[] k, final TIdSet v[], final Comparator<? super Float> c) {
        this(c);
     ...
     * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
     */
    public Float2TidSetLTTreeAggregateMap(final float[] k, final TIdSet v[]) {
        this(k, v, null);
    }
    /*
     ...
     * key exists.
     */
    final AggregateEntry findKey(final float k) {
        AggregateEntry e = tree;
        int cmp;
        while (e != null && (cmp = compare(k, e.getFloatKey())) != 0)
            e = cmp < 0 ? e.left() : e.right();
        return e;
    }

    public TIdSet findSmallerTIdsImpl(final float k, boolean inclusive) {
        AggregateEntry e = tree;
        int cmp;
    ...
    private Operand findSmallerTIdsImplFastAgg(final float k, boolean inclusive) {
        ...
        while (e != null && (cmp = compare(k, e.getFloatKey())) != 0) {
            if (cmp < 0) {
                ...
                if (cachedSet == null || e.getFloatKey() >= previousK) {
     ...
     */
    final AggregateEntry locateKey(final float k) {
    ...
    @Override
    public TIdSet put(final float k, final TIdSet v) {
    ...
    public void addAndCreateIfNecessaryImpl(final float k, final int tId) {
        ...
        if (tree == null) { // The case of the empty tree is treated separately.
            ...
            int cmp, i = 0;
            while (true) {
                if ((cmp = compare(k, p.getFloatKey())) == 0) {
     ...
     */
    private AggregateEntry add(final float k) {
        /*
    ...
    public void removeTIdAndSetIfNecessaryImpl(final float k, final int tId) {
        modified = false;
        if (tree == null) return;
        AggregateEntry p = tree;
        int cmp;
        int i = 0;
        while ((cmp = compare(k, p.getFloatKey())) != 0) {
            dirPath[i] = cmp > 0;
            nodePath[i] = p;
            if (dirPath[i++]) {
                if ((p = p.right()) == null) {
    ...
    @Override
    public TIdSet remove(final float k) {
        modified = false;
        if (tree == null)
            return defRetValue;
        AggregateEntry p = tree;
        ...
        final float kk = k;
        while (true) {
            if ((cmp = compare(kk, p.getFloatKey())) == 0)
                break;
            dirPath[i] = cmp > 0;
            nodePath[i] = p;
            if (dirPath[i++]) {
    ...
    public Operand findSmallerTIds(Number k, boolean inclusive) {
        return findSmallerTIdsImplFastAgg((float) k, inclusive);
    }

    @Override
    public void addAndCreateIfNecessary(Number k, int tId) {
        addAndCreateIfNecessaryImpl((Float) k, tId);
        if ((Float) k < previousK) {
            cachedSet.add(tId);
        }
    }
    ...
    public void removeTIdAndSetIfNecessary(Number k, int tId) {
        removeTIdAndSetIfNecessaryImpl((Float) k, tId);
        if ((Float) k < previousK) {
     ...
     *
     * <p>
     * We use the only "metadata", i.e., {@link AggregateEntry#info}, to store information
     ...
     * tree.
     */
    private static final class AggregateEntry extends AbstractFloat2ObjectMap.BasicEntry<TIdSet> implements Cloneable {
        /**
         ...
         * @param v a value.
         */
        AggregateEntry(final float k, final TIdSet v) {
            super(k, v);
        ...
        public boolean equals(final Object o) {
            if (!(o instanceof Map.Entry))
                return false;
            Map.Entry<Float, TIdSet> e = (Map.Entry<Float, TIdSet>) o;
    ...
    @Override
    public TIdSet get(final float k) {
    ...
    private class TreeIterator {
        ...
        TreeIterator(final float k) {
            if ((next = locateKey(k)) != null) {
                if (compare(next.getFloatKey(), k) <= 0) {
                    prev = next;
                    next = next.next();
                } else
                    prev = next.prev();
        ...
        public void remove() {
            ...
            updatePrevious();
            updateNext();
            Float2TidSetLTTreeAggregateMap.this.remove(curr.getFloatKey());
            curr = null;
        }
     ...
     * This class can iterate in both directions on a threaded tree.
     */
    private class EntryIterator extends TreeIterator implements ObjectListIterator<Float2ObjectMap.Entry<TIdSet>> {
        EntryIterator() {
        }
        ...
        @Override
        public Float2ObjectMap.Entry<TIdSet> next() {
            return nextEntry();
        }

        @Override
        public Float2ObjectMap.Entry<TIdSet> previous() {
            return previousEntry();
        }
    }

    @Override
    public ObjectSortedSet<Float2ObjectMap.Entry<TIdSet>> float2ObjectEntrySet() {
        if (entries == null)
            entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<TIdSet>>() {
                final Comparator<? super Float2ObjectMap.Entry<TIdSet>> comparator = (Comparator<Float2ObjectMap.Entry<TIdSet>>) (
                        x, y) -> Float2TidSetLTTreeAggregateMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());

                @Override
                public Comparator<? super Float2ObjectMap.Entry<TIdSet>> comparator() {
                    return comparator;
                }

                @Override
                public ObjectBidirectionalIterator<Float2ObjectMap.Entry<TIdSet>> iterator() {
                    return new EntryIterator();
                }

                @Override
                public ObjectBidirectionalIterator<Float2ObjectMap.Entry<TIdSet>> iterator(
                        final Float2ObjectMap.Entry<TIdSet> from) {
                ...
                public boolean contains(final Object o) {
                    ...
                    if (e.getKey() == null || !(e.getKey() instanceof Float))
                        return false;
                    final AggregateEntry f = findKey(((Float) (e.getKey())).floatValue());
                    return e.equals(f);
                }
                ...
                public boolean remove(final Object o) {
                    ...
                    if (f == null || !java.util.Objects.equals(f.getValue(), (e.getValue())))
                        return false;
                    Float2TidSetLTTreeAggregateMap.this.remove(f.getFloatKey());
                    return true;
                }
                ...
                @Override
                public void clear() {
                    Float2TidSetLTTreeAggregateMap.this.clear();
                }

                @Override
                public Float2ObjectMap.Entry<TIdSet> first() {
                    return firstEntry;
                }

                @Override
                public Float2ObjectMap.Entry<TIdSet> last() {
                    return lastEntry;
                }

                @Override
                public ObjectSortedSet<Float2ObjectMap.Entry<TIdSet>> subSet(Float2ObjectMap.Entry<TIdSet> from,
                                                                             Float2ObjectMap.Entry<TIdSet> to) {
                    return subMap(from.getFloatKey(), to.getFloatKey()).float2ObjectEntrySet();
                }

                @Override
                public ObjectSortedSet<Float2ObjectMap.Entry<TIdSet>> headSet(Float2ObjectMap.Entry<TIdSet> to) {
                    return headMap(to.getFloatKey()).float2ObjectEntrySet();
                }

                @Override
                public ObjectSortedSet<Float2ObjectMap.Entry<TIdSet>> tailSet(Float2ObjectMap.Entry<TIdSet> from) {
    ...
    private final class KeyIterator extends TreeIterator implements FloatListIterator {
        ...
        public float previousFloat() {
            return previousEntry().getFloatKey();
        }
    }
     ...
     */
    private class KeySet extends AbstractFloat2ObjectSortedMap<TIdSet>.KeySet {
        @Override
    ...
    public ObjectCollection<TIdSet> values() {
        if (values == null)
            values = new AbstractObjectCollection<TIdSet>() {
                @Override
    ...
    @Override
    public Float2ObjectSortedMap<TIdSet> headMap(float to) {
        return new Submap((0), true, to, false);
    }

    @Override
    public Float2ObjectSortedMap<TIdSet> tailMap(float from) {
        return new Submap(from, false, (0), true);
    }

    @Override
    public Float2ObjectSortedMap<TIdSet> subMap(float from, float to) {
     ...
     */
    private final class Submap extends AbstractFloat2ObjectSortedMap<TIdSet> implements java.io.Serializable {
        private static final long serialVersionUID = -7046029254386353129L;
        /**
        ...
        public Submap(final float from, final boolean bottom, final float to, final boolean top) {
            if (!bottom && !top && Float2TidSetLTTreeAggregateMap.this.compare(from, to) > 0)
                throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
            this.from = from;
            this.bottom = bottom;
            this.to = to;
            this.top = top;
            this.defRetValue = Float2TidSetLTTreeAggregateMap.this.defRetValue;
        }
        ...
        final boolean in(final float k) {
            return (bottom || Float2TidSetLTTreeAggregateMap.this.compare(k, from) >= 0)
                    && (top || Float2TidSetLTTreeAggregateMap.this.compare(k, to) < 0);
        }
        ...
        public ObjectSortedSet<Float2ObjectMap.Entry<TIdSet>> float2ObjectEntrySet() {
            if (entries == null)
                entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<TIdSet>>() {
                    ...
                    @Override
                    public Comparator<? super Float2ObjectMap.Entry<TIdSet>> comparator() {
                        return Float2TidSetLTTreeAggregateMap.this.float2ObjectEntrySet().comparator();
                    }
                    ...
                    public boolean contains(final Object o) {
                        ...
                        if (e.getKey() == null || !(e.getKey() instanceof Float))
                            return false;
                        final Float2TidSetLTTreeAggregateMap.AggregateEntry f = findKey(((Float) (e.getKey())).floatValue());
                        return f != null && in(f.getFloatKey()) && e.equals(f);
                    }
                    ...
                    public boolean remove(final Object o) {
                        ...
                        if (e.getKey() == null || !(e.getKey() instanceof Float))
                            return false;
                        final Float2TidSetLTTreeAggregateMap.AggregateEntry f = findKey(((Float) (e.getKey())).floatValue());
                        if (f != null && in(f.getFloatKey()))
                            Submap.this.remove(f.getFloatKey());
                        return f != null;
                    }
        ...
        public boolean containsKey(final float k) {
            return in(k) && Float2TidSetLTTreeAggregateMap.this.containsKey(k);
        }
        ...
        public TIdSet get(final float k) {
            final Float2TidSetLTTreeAggregateMap.AggregateEntry e;
            final float kk = k;
        ...
        public TIdSet put(final float k, final TIdSet v) {
            ...
            final TIdSet oldValue = Float2TidSetLTTreeAggregateMap.this.put(k, v);
        ...
        public TIdSet remove(final float k) {
            ...
            final TIdSet oldValue = Float2TidSetLTTreeAggregateMap.this.remove(k);
        ...
        @Override
        public Float2ObjectSortedMap<TIdSet> headMap(final float to) {
            if (top)
        ...
        @Override
        public Float2ObjectSortedMap<TIdSet> tailMap(final float from) {
            if (bottom)
         ...
         * empty.
         */
        public Float2TidSetLTTreeAggregateMap.AggregateEntry lastEntry() {
            if (tree == null)
                return null;
            ...
            if (top)
                ...
                if (compare(e.getFloatKey(), to) >= 0)
                    e = e.prev();
            }
            // Finally, if this submap doesn't go to -infinity, we check that the resulting
            // key isn't smaller than the start.
            if (e == null || !bottom && compare(e.getFloatKey(), from) < 0)
                return null;
            return e;
        }
        ...
        @Override
        public float firstFloatKey() {
            Float2TidSetLTTreeAggregateMap.AggregateEntry e = firstEntry();
            if (e == null)
                throw new NoSuchElementException();
        ...
        @Override
        public float lastFloatKey() {
            Float2TidSetLTTreeAggregateMap.AggregateEntry e = lastEntry();
            if (e == null)
                throw new NoSuchElementException();
            return e.getFloatKey();
        }
        ...
        private class SubmapIterator extends TreeIterator {
            ...
            SubmapIterator(final float k) {
                this();
                if (next != null) {
                    if (!bottom && compare(k, next.getFloatKey()) < 0)
                        prev = null;
                    else if (!top && compare(k, (prev = lastEntry()).getFloatKey()) >= 0)
                        next = null;
                    else {
                        next = locateKey(k);
                        if (compare(next.getFloatKey(), k) <= 0) {
            ...
            void updatePrevious() {
                prev = prev.prev();
                if (!bottom && prev != null && Float2TidSetLTTreeAggregateMap.this.compare(prev.getFloatKey(), from) < 0)
                    prev = null;
            }
            ...
            void updateNext() {
                next = next.next();
                if (!top && next != null && Float2TidSetLTTreeAggregateMap.this.compare(next.getFloatKey(), to) >= 0)
                    next = null;
            }
        }
        ...
        private class SubmapEntryIterator extends SubmapIterator
                implements
                ObjectListIterator<Float2ObjectMap.Entry<TIdSet>> {
            SubmapEntryIterator() {
            }
    ...
    @Override
    @SuppressWarnings("unchecked")
    public Float2TidSetLTTreeAggregateMap clone() {
        Float2TidSetLTTreeAggregateMap c;
        try {
            c = (Float2TidSetLTTreeAggregateMap) super.clone();
        } catch (CloneNotSupportedException cantHappen) {
            throw new InternalError();
        }
        c.keys = null;
