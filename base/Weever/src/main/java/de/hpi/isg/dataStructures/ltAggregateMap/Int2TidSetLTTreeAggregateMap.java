...
import de.hpi.isg.tidSets.RoaringTidSet;
 ...
 * Changed by me!
 */
public class Int2TidSetLTTreeAggregateMap extends AbstractInt2ObjectSortedMap<TIdSet> implements java.io.Serializable, Cloneable, LTAggregateMap {

    /**
     * A reference to the root entry.
     */
    protected transient AggregateEntry tree;
    /**
     ...
     */
    protected transient AggregateEntry firstEntry;
    /**
     * The last key in this map.
     */
    protected transient AggregateEntry lastEntry;
    /**
     * Cached set of entries.
     */
    protected transient ObjectSortedSet<Int2ObjectMap.Entry<TIdSet>> entries;
    /**
     * Cached set of keys.
     */
     ...
     */
    protected transient ObjectCollection<TIdSet> values;
    /**
     ...
     */
    public Int2TidSetLTTreeAggregateMap() {
        tree = null;
        count = 0;
    }
     ...
     */
    public Int2TidSetLTTreeAggregateMap(final Comparator<? super Integer> c) {
        this();
        storedComparator = c;
        setActualComparator();
    }
     ...
     */
    public Int2TidSetLTTreeAggregateMap(final Map<? extends Integer, ? extends TIdSet> m) {
        this();
        putAll(m);
    }
     ...
     */
    public Int2TidSetLTTreeAggregateMap(final SortedMap<Integer, TIdSet> m) {
        this(m.comparator());
        putAll(m);
    }
     ...
     */
    public Int2TidSetLTTreeAggregateMap(final Int2ObjectMap<? extends TIdSet> m) {
        this();
        putAll(m);
    }
     ...
     */
    public Int2TidSetLTTreeAggregateMap(final Int2ObjectSortedMap<TIdSet> m) {
        this(m.comparator());
        putAll(m);
    }
     ...
     */
    public Int2TidSetLTTreeAggregateMap(final int[] k, final TIdSet[] v, final Comparator<? super Integer> c) {
        this(c);
     ...
     * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
     */
    public Int2TidSetLTTreeAggregateMap(final int[] k, final TIdSet[] v) {
        this(k, v, null);
    }
    /*
     ...
     */
    final AggregateEntry findKey(final int k) {
        AggregateEntry e = tree;
        int cmp;
        while (e != null && (cmp = compare(k, e.getIntKey())) != 0) e = cmp < 0 ? e.left() : e.right();
        return e;
    }

    public TIdSet findSmallerTIdsImpl(final int k, boolean inclusive) {
        AggregateEntry e = tree;
        int cmp;
        TIdSet result = Utils.createNewTIdSet();
        while (e != null && (cmp = compare(k, e.getIntKey())) != 0) {
            if (cmp < 0) {
                e = e.left();
            } else {
                result.union(e.ltAggregate).union(e.getValue());
                e = e.right();
            }
        }
        if (e != null) {
            result.union(e.ltAggregate);
            if (inclusive) {
                result.union(e.getValue());
            }
        }
        return result;
    }

    public Operand findSmallerTIdsImplFastAgg(final int k, boolean inclusive) {
        ArrayList<RoaringBitmap> results = new ArrayList<>(32);
        AggregateEntry e = tree;
        int cmp;
        if (k < previousK) {
            cachedSet = null;
        ...
        while (e != null && (cmp = compare(k, e.getIntKey())) != 0) {
            if (cmp < 0) {
                ...
                if (cachedSet == null || e.getIntKey() >= previousK) {
                    results.add(e.ltAggregate.getRoaring());
                    results.add(e.getValue().getRoaring());
                }
                e = e.right();
            }
        }
        TIdSet equalVals = null;
        if (e != null) {
            results.add(e.ltAggregate.getRoaring());
            equalVals = e.getValue();
            if (inclusive) {
                results.add(e.getValue().getRoaring());
            }
        }
        if (cachedSet != null) {
            results.add(cachedSet.getRoaring());
        }
        cachedSet = new RoaringTidSet(FastAggregation.or(results.iterator()));
        previousK = k;
        return new Operand(cachedSet, equalVals, inclusive);
    }
     ...
     */
    final AggregateEntry locateKey(final int k) {
        AggregateEntry e = tree, last = tree;
        int cmp = 0;
        while (e != null && (cmp = compare(k, e.getIntKey())) != 0) {
            last = e;
    ...
    private transient AggregateEntry[] nodePath;

    private void allocatePaths() {
        dirPath = new boolean[64];
        nodePath = new AggregateEntry[64];
    }

    @Override
    public TIdSet put(final int k, final TIdSet v) {
        AggregateEntry e = add(k);
        final TIdSet oldValue = e.getValue();
        e.setValue(v);
        return oldValue;
    }

    public void addAndCreateIfNecessaryImpl(final int k, final int tId) {
        ...
        if (tree == null) { // The case of the empty tree is treated separately.
            count++;
            tree = lastEntry = firstEntry = new AggregateEntry(k, Utils.createNewTIdSet().add(tId));
        } else {
            AggregateEntry e;
            AggregateEntry p = tree;
            int cmp, i = 0;
            while (true) {
                if ((cmp = compare(k, p.getIntKey())) == 0) {
                ...
                if (dirPath[i++] = cmp > 0) {
                    if (p.succ()) {
                        count++;
                        e = new AggregateEntry(k, Utils.createNewTIdSet().add(tId));
            ...
            while (i > 0 && !nodePath[i].black()) {
                if (!dirPath[i - 1]) {
                    AggregateEntry y = nodePath[i - 1].right;
                    if (!nodePath[i - 1].succ() && !y.black()) {
                        ...
                        if (!dirPath[i]) y = nodePath[i];
                            ...
                            x.right = y.left;
                            y.left = x;
                            y.ltAggregate.union(x.ltAggregate).union(x.getValue());
                            nodePath[i - 1].left = y;
                            if (y.pred()) {
                                y.pred(false);
                                x.succ(y);
                        ...
                        y.black(true);
                        x.left = y.right;
                        (x.ltAggregate.minus(y.ltAggregate)).minus(y.getValue());
                        y.right = x;
                        if (i < 2) tree = y;
                        else {
                    ...
                    AggregateEntry y = nodePath[i - 1].left;
     ...
     */
    private AggregateEntry add(final int k) {
        /* After execution of this method, modified is true iff a new entry has been inserted. */
        ...
        if (tree == null) { // The case of the empty tree is treated separately.
            count++;
            e = tree = lastEntry = firstEntry = new AggregateEntry(k, defRetValue);
        } else {
            AggregateEntry p = tree;
            int cmp, i = 0;
            while (true) {
                ...
                if (dirPath[i++] = cmp > 0) {
                    if (p.succ()) {
                        count++;
                        e = new AggregateEntry(k, defRetValue);
                        if (p.right == null) lastEntry = e;
                        e.left = p;
                        e.right = p.right;
                        p.right(e);
    ...
    public void removeTIdAndSetIfNecessaryImpl(final int k, final int tId) {
        modified = false;
        if (tree == null) return;
        AggregateEntry p = tree;
        int cmp;
        int i = 0;
        while ((cmp = compare(k, p.getIntKey())) != 0) {
            dirPath[i] = cmp > 0;
            nodePath[i] = p;
            if (dirPath[i++]) {
                if ((p = p.right()) == null) {
        ...
        // only remove node if set is empty
        if (!p.getValue().remove(tId).isEmpty()) {
            return;
        }
        ...
        if (p.succ()) {
            ...
            boolean color;
            AggregateEntry r = p.right;
            if (r.pred()) {
                // case 3
                r.left = p.left;
                // TODO check this case
                r.ltAggregate = p.ltAggregate;
                r.pred(p.pred());
        ...
        if (p.black()) {
            for (; i > 0; i--) {
                if (dirPath[i - 1] && !nodePath[i - 1].succ() ||
                    ...
                    AggregateEntry x = dirPath[i - 1] ? nodePath[i - 1].right : nodePath[i - 1].left;
                    if (!x.black()) {
                        x.black(true);
                        break;
                    }
                ...
                if (!dirPath[i - 1]) {
                    AggregateEntry w = nodePath[i - 1].right;
                    if (!w.black()) {
                        w.black(true);
                        nodePath[i - 1].black(false);
                        nodePath[i - 1].right = w.left;
                        w.left = nodePath[i - 1];
                        w.ltAggregate.union(nodePath[i - 1].ltAggregate).union(nodePath[i - 1].getValue());
                        if (i < 2) tree = w;
                        else {
                            if (dirPath[i - 2]) {
                                ...
                                nodePath[i - 2].left = w;
                                // TODO check whether we need to update leftAggregate - does not seem likely
                                // nodePath[i - 2].leftAggregate = Utils.createNewTIdSet().union(w.leftAggregate).union(w.rightAggregate).union(w.getValue());
                            }
                        }
                    ...
                    if ((w.pred() || w.left.black()) &&
                        ...
                        if (w.succ() || w.right.black()) {
                            AggregateEntry y = w.left;
                            y.black(true);
                            w.black(false);
                            w.left = y.right;
                            (w.ltAggregate.minus(y.ltAggregate)).minus(y.getValue());
                            y.right = w;
                    ...
                    AggregateEntry w = nodePath[i - 1].left;
                    if (!w.black()) {
                        w.black(true);
                        nodePath[i - 1].black(false);
                        nodePath[i - 1].left = w.right;
                        (nodePath[i - 1].ltAggregate.minus(w.ltAggregate)).minus(w.getValue());
                    ...
                    if ((w.pred() || w.left.black()) &&
                        ...
                        if (w.pred() || w.left.black()) {
                            AggregateEntry y = w.right;
                            y.black(true);
                            w.black(false);
                            w.right = y.left;
                            y.left = w;
                            y.ltAggregate.union(w.ltAggregate).union(w.getValue());
    ...
    @Override
    public TIdSet remove(final int k) {
        modified = false;
        ...
        final int kk = k;
        while (true) {
            if ((cmp = compare(kk, p.getIntKey())) == 0) break;
            dirPath[i] = cmp > 0;
            nodePath[i] = p;
            if (dirPath[i++]) {
                if ((p = p.right()) == null) {
    ...
    @Override
    public Operand findSmallerTIds(Number k, boolean inclusive) {
        return findSmallerTIdsImplFastAgg((Integer) k, inclusive);
    }

    @Override
    public void addAndCreateIfNecessary(Number k, int tId) {
        addAndCreateIfNecessaryImpl((int) k, tId);
        if ((int) k < previousK) {
            cachedSet.add(tId);
        }
    }
    ...
    @Override
    public void removeTIdAndSetIfNecessary(Number k, int tId) {
        removeTIdAndSetIfNecessaryImpl((Integer) k, tId);
        if ((int) k < previousK) {
            cachedSet.remove(tId);
        }
    }
     ...
     *
     * <p>We use the only "metadata", i.e., {@link AggregateEntry#info}, to store
     ...
     */
    private static final class AggregateEntry extends AbstractInt2ObjectMap.BasicEntry<TIdSet> implements Cloneable {
        /**
         * The bit in this mask is true, the node is black.
         */
         ...
         */
        AggregateEntry left, right;

        /**
         * The aggregates of the left and right subtrees.
         */
        TIdSet ltAggregate = Utils.createNewTIdSet();

        /**
         * This integer holds different information in different bits (see {@link #SUCC_MASK} and {@link #PRED_MASK}.
         */
        int info;
         ...
         * @param v a value.
         */
        AggregateEntry(final int k, final TIdSet v) {
            super(k, v);
         ...
         */
        void pred(final AggregateEntry pred) {
            info |= PRED_MASK;
            left = pred;
        }
         ...
         */
        void succ(final AggregateEntry succ) {
            info |= SUCC_MASK;
            right = succ;
        }
         ...
         */
        void left(final AggregateEntry left) {
            info &= ~PRED_MASK;
            this.left = left;
        }
         ...
         */
        void right(final AggregateEntry right) {
            info &= ~SUCC_MASK;
            this.right = right;
        }
         ...
         */
        AggregateEntry next() {
            AggregateEntry next = this.right;
         ...
         */
        AggregateEntry prev() {
            AggregateEntry prev = this.left;
            if ((info & PRED_MASK) == 0) while ((prev.info & SUCC_MASK) == 0) prev = prev.right;
            return prev;
        }

        @Override
        public TIdSet setValue(final TIdSet value) {
            final TIdSet oldValue = this.value;
            this.value = value;
            return oldValue;
        }
        ...
        public boolean equals(final Object o) {
            ...
            Map.Entry<Integer, TIdSet> e = (Map.Entry<Integer, TIdSet>) o;
    ...
    private void print(String prefix, AggregateEntry n, boolean isLeft) {
        if (n != null) {
            System.out.println(prefix + (isLeft ? "|-- " : "\\-- ") + n.getIntKey());
    ...
    @Override
    public TIdSet get(final int k) {
        final AggregateEntry e = findKey(k);
        return e == null ? defRetValue : e.getValue();
    }
    ...
    private class TreeIterator {
	  ...
	  /* If the last operation was a next(), we are removing an entry that preceeds
            ...
            updatePrevious();
            updateNext();
            Int2TidSetLTTreeAggregateMap.this.remove(curr.getIntKey());
            curr = null;
        }
     ...
     * <p>This class can iterate in both directions on a threaded tree.
     */
    private class EntryIterator extends TreeIterator implements ObjectListIterator<Int2ObjectMap.Entry<TIdSet>> {
        EntryIterator() {
        }
        ...
        @Override
        public Int2ObjectMap.Entry<TIdSet> next() {
            return nextEntry();
        }

        @Override
        public Int2ObjectMap.Entry<TIdSet> previous() {
    ...
    public ObjectSortedSet<Int2ObjectMap.Entry<TIdSet>> int2ObjectEntrySet() {
        if (entries == null) entries = new AbstractObjectSortedSet<Int2ObjectMap.Entry<TIdSet>>() {
            final Comparator<? super Int2ObjectMap.Entry<TIdSet>> comparator = (Int2TidSetLTTreeAggregateMap.this.actualComparator == null ?
                    (Comparator<Int2ObjectMap.Entry<TIdSet>>) (x, y) -> (Integer.compare((x.getIntKey()), (y.getIntKey()))) :
                    (Comparator<Int2ObjectMap.Entry<TIdSet>>) (x, y) -> Int2TidSetLTTreeAggregateMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey())
            );

            @Override
            public Comparator<? super Int2ObjectMap.Entry<TIdSet>> comparator() {
                return comparator;
            }

            @Override
            public ObjectBidirectionalIterator<Int2ObjectMap.Entry<TIdSet>> iterator() {
                return new EntryIterator();
            }

            @Override
            public ObjectBidirectionalIterator<Int2ObjectMap.Entry<TIdSet>> iterator(final Int2ObjectMap.Entry<TIdSet> from) {
            ...
            public boolean contains(final Object o) {
                ...
                final AggregateEntry f = findKey(((Integer) (e.getKey())).intValue());
                return e.equals(f);
            }
            ...
            public boolean remove(final Object o) {
                ...
                Int2TidSetLTTreeAggregateMap.this.remove(f.getIntKey());
                return true;
            }
            ...
            @Override
            public void clear() {
                Int2TidSetLTTreeAggregateMap.this.clear();
            }

            @Override
            public Int2ObjectMap.Entry<TIdSet> first() {
                return firstEntry;
            }

            @Override
            public Int2ObjectMap.Entry<TIdSet> last() {
                return lastEntry;
            }

            @Override
            public ObjectSortedSet<Int2ObjectMap.Entry<TIdSet>> subSet(Int2ObjectMap.Entry<TIdSet> from, Int2ObjectMap.Entry<TIdSet> to) {
                return subMap(from.getIntKey(), to.getIntKey()).int2ObjectEntrySet();
            }

            @Override
            public ObjectSortedSet<Int2ObjectMap.Entry<TIdSet>> headSet(Int2ObjectMap.Entry<TIdSet> to) {
                return headMap(to.getIntKey()).int2ObjectEntrySet();
            }

            @Override
            public ObjectSortedSet<Int2ObjectMap.Entry<TIdSet>> tailSet(Int2ObjectMap.Entry<TIdSet> from) {
    ...
    private final class KeyIterator extends TreeIterator implements IntListIterator {
        ...
        public int previousInt() {
            return previousEntry().getIntKey();
        }
    }
     ...
     */
    private class KeySet extends AbstractInt2ObjectSortedMap<TIdSet>.KeySet {
        @Override
        public IntBidirectionalIterator iterator() {
            return new Int2TidSetLTTreeAggregateMap.KeyIterator();
        }

        @Override
        public IntBidirectionalIterator iterator(final int from) {
            return new Int2TidSetLTTreeAggregateMap.KeyIterator(from);
        }
    }
    ...
    public IntSortedSet keySet() {
        if (keys == null) keys = new Int2TidSetLTTreeAggregateMap.KeySet();
        return keys;
    }
     ...
     */
    private final class ValueIterator extends TreeIterator implements ObjectListIterator<TIdSet> {
        @Override
        public TIdSet next() {
     ...
     */
    @Override
    public ObjectCollection<TIdSet> values() {
        if (values == null) values = new AbstractObjectCollection<TIdSet>() {
            @Override
            public ObjectIterator<TIdSet> iterator() {
    ...
    @Override
    public Int2ObjectSortedMap<TIdSet> headMap(int to) {
        return new Submap((0), true, to, false);
    }

    @Override
    public Int2ObjectSortedMap<TIdSet> tailMap(int from) {
        return new Submap(from, false, (0), true);
    }

    @Override
    public Int2ObjectSortedMap<TIdSet> subMap(int from, int to) {
     ...
     * on-the-fly.
     */
    private final class Submap extends AbstractInt2ObjectSortedMap<TIdSet> implements java.io.Serializable {
        private static final long serialVersionUID = -7046029254386353129L;
        /**
        ...
        public Submap(final int from, final boolean bottom, final int to, final boolean top) {
            if (!bottom && !top && Int2TidSetLTTreeAggregateMap.this.compare(from, to) > 0)
                throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
            this.from = from;
            this.bottom = bottom;
            this.to = to;
            this.top = top;
            this.defRetValue = Int2TidSetLTTreeAggregateMap.this.defRetValue;
        }
        ...
        final boolean in(final int k) {
            return (bottom || Int2TidSetLTTreeAggregateMap.this.compare(k, from) >= 0) &&
                    (top || Int2TidSetLTTreeAggregateMap.this.compare(k, to) < 0);
        }
        ...
        public ObjectSortedSet<Int2ObjectMap.Entry<TIdSet>> int2ObjectEntrySet() {
            if (entries == null) entries = new AbstractObjectSortedSet<Int2ObjectMap.Entry<TIdSet>>() {
                ...
                @Override
                public ObjectBidirectionalIterator<Int2ObjectMap.Entry<TIdSet>> iterator(final Int2ObjectMap.Entry<TIdSet> from) {
                    return new Submap.SubmapEntryIterator(from.getIntKey());
                }

                @Override
                public Comparator<? super Int2ObjectMap.Entry<TIdSet>> comparator() {
                    return Int2TidSetLTTreeAggregateMap.this.int2ObjectEntrySet().comparator();
                }
                ...
                public boolean contains(final Object o) {
                    ...
                    final AggregateEntry f = findKey(((Integer) (e.getKey())).intValue());
                    return f != null && in(f.getIntKey()) && e.equals(f);
                }
                ...
                public boolean remove(final Object o) {
                    ...
                    if (e.getKey() == null || !(e.getKey() instanceof Integer)) return false;
                    final AggregateEntry f = findKey(((Integer) (e.getKey())).intValue());
                    if (f != null && in(f.getIntKey()))
                        Submap.this.remove(f.getIntKey());
                    return f != null;
                }
        ...
        public boolean containsKey(final int k) {
            ...
            return in(k) && Int2TidSetLTTreeAggregateMap.this.containsKey(k);
        }
        ...
        public TIdSet get(final int k) {
            final AggregateEntry e;
            final int kk = k;
            return in(kk) && (e = findKey(kk)) != null ? e.getValue() : this.defRetValue;
        }
        ...
        public TIdSet put(final int k, final TIdSet v) {
            ...
            final TIdSet oldValue = Int2TidSetLTTreeAggregateMap.this.put(k, v);
        ...
        public TIdSet remove(final int k) {
            ...
            final TIdSet oldValue = Int2TidSetLTTreeAggregateMap.this.remove(k);
        ...
        @Override
        public Int2ObjectSortedMap<TIdSet> headMap(final int to) {
            if (top) return new Submap(from, bottom, to, false);
            return compare(to, this.to) < 0 ? new Submap(from, bottom, to, false) : this;
        }

        @Override
        public Int2ObjectSortedMap<TIdSet> tailMap(final int from) {
         ...
         */
        public AggregateEntry firstEntry() {
         ...
         */
        public AggregateEntry lastEntry() {
            if (tree == null) return null;
            ...
            if (top) e = lastEntry;
                ...
                if (compare(e.getIntKey(), to) >= 0) e = e.prev();
            }
            // Finally, if this submap doesn't go to -infinity, we check that the resulting key isn't smaller than the start.
            if (e == null || !bottom && compare(e.getIntKey(), from) < 0) return null;
            return e;
        }

        @Override
        public int firstIntKey() {
            AggregateEntry e = firstEntry();
        ...
        private class SubmapIterator extends TreeIterator {
            ...
            SubmapIterator(final int k) {
                this();
                if (next != null) {
                    if (!bottom && compare(k, next.getIntKey()) < 0) prev = null;
                    else if (!top && compare(k, (prev = lastEntry()).getIntKey()) >= 0) next = null;
                    else {
                        next = locateKey(k);
                        if (compare(next.getIntKey(), k) <= 0) {
                            prev = next;
            ...
            void updatePrevious() {
                prev = prev.prev();
                if (!bottom && prev != null && Int2TidSetLTTreeAggregateMap.this.compare(prev.getIntKey(), from) < 0)
                    prev = null;
            }
            ...
            void updateNext() {
                next = next.next();
                if (!top && next != null && Int2TidSetLTTreeAggregateMap.this.compare(next.getIntKey(), to) >= 0)
                    next = null;
            }
        }

        private class SubmapEntryIterator extends SubmapIterator implements ObjectListIterator<Int2ObjectMap.Entry<TIdSet>> {
            SubmapEntryIterator() {
            }
         ...
         * entries.
         */
        private final class SubmapValueIterator extends SubmapIterator implements ObjectListIterator<TIdSet> {
            @Override
            public TIdSet next() {
                return nextEntry().getValue();
            }
    ...
    @Override
    @SuppressWarnings("unchecked")
    public Int2TidSetLTTreeAggregateMap clone() {
        Int2TidSetLTTreeAggregateMap c;
        try {
            c = (Int2TidSetLTTreeAggregateMap) super.clone();
        } catch (CloneNotSupportedException cantHappen) {
            throw new InternalError();
        }
        c.keys = null;
        ...
        if (count != 0) {
            ...
            AggregateEntry e, p, q, rp = new AggregateEntry(), rq = new AggregateEntry();
            p = rp;
            rp.left(tree);
            q = rq;
            rq.pred(null);
