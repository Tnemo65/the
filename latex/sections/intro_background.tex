...
\textbf{GAP 1 -- Chi phí phát hiện DC trên luồng.}

Phát hiện vi phạm denial constraints yêu cầu đối chiếu đa bản ghi trong cùng cửa sổ thời gian. Cách tiếp cận brute-force có độ phức tạp $O(N^2)$, không scale khi kích thước cửa sổ tăng. Thêm vào đó, đến 95\% constraints được tự động suy luận là FALSE khi thiếu ngữ cảnh, dẫn đến chi phí xử lý phí không cần thiết.

Nguồn: VLDB 2025 Martin et al., Incremental DC Discovery \cite{springer2024incremental}, Stream DaQ \cite{arxiv2506streamdaq}.

\textbf{GAP 2 -- Luật tĩnh không thích nghi với concept drift.}

Các luật kiểm tra được định nghĩa bằng ngưỡng cố định (static thresholds). Khi phân phối dữ liệu thay đổi theo thời gian (kẹt xe, mưa bão, sự kiện đặc biệt), ngưỡng cố định không phản ánh ngữ cảnh thực tế, dẫn đến tỷ lệ false positive cao bất thường trong thời gian drift.

Nguồn: Rapidash \cite{vldb2024rapidash}, Stream DaQ \cite{arxiv2506streamdaq}.

\textbf{GAP 3 -- Thiếu cơ chế provisional--retract trong môi trường watermark.}

Trong môi trường streaming thực, dữ liệu đến muộn (late arrivals) và out-of-order arrivals rất phổ biến do stragglers, độ trễ mạng biến thiên, và các partition rỗng không produce. Nếu hệ thống thiếu cơ chế provisional alert và retraction, mọi cảnh báo đều bị coi là chính thức ngay khi phát hiện, dẫn đến false alert tăng mạnh khi watermark bị lag.
