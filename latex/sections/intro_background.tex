...
\section{Công cụ DQ thị trường hiện tại}

Phần này đánh giá các công cụ DQ trên thị trường (2024-2025) theo năm chiều: hỗ trợ kiểm tra denial constraints, ngữ nghĩa watermark, xử lý late data, cập nhật gia tăng và streaming native.

{\small
\begin{longtable}{|L{0.20\textwidth}|c|c|c|c|c|}
\hline
\textbf{Nền tảng} & \rotatebox{70}{DC Check} & \rotatebox{70}{Watermark} & \rotatebox{70}{Late Data} & \rotatebox{70}{Incremental} & \rotatebox{70}{Streaming} \\
\hline
Apache Griffin & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{yellow!25}\ding{63} & \cellcolor{yellow!25}\ding{63} \\
\hline
Great Expectations & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} \\
\hline
Soda Core & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} \\
\hline
Spark Structured Streaming & \cellcolor{red!25}\ding{55} & \cellcolor{green!25}\ding{51} & \cellcolor{yellow!25}\ding{63} & \cellcolor{yellow!25}\ding{63} & \cellcolor{green!25}\ding{51} \\
\hline
AWS Deequ / Sagemaker & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{yellow!25}\ding{63} & \cellcolor{red!25}\ding{55} \\
\hline
GCP Dataplex & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} \\
\hline
Monte Carlo / Bigeye & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} & \cellcolor{red!25}\ding{55} \\
\hline
\textbf{Ghi chú ký hiệu:} \ding{51} = hỗ trợ \,|\, \ding{63} = hỗ trợ hạn chế \,|\, \ding{55} = không hỗ trợ \\
\hline
\end{longtable}
}

\textbf{Phân tích chi tiết từng nền tảng.}

\medskip\noindent\textbf{Apache Griffin} \cite{griffin}: Apache top-level project (2018, hiện archived). Điểm mạnh: 30+ built-in checks, Kafka integration, hỗ trợ batch và streaming. Điểm yếu: không DC checking, không watermark, dự án không còn được bảo trì. Khác biệt WAVES: DC engine riêng, watermark-aware decision layer.

\medskip\noindent\textbf{Great Expectations} \cite{greatexp}: Thư viện Python mã nguồn mở. Điểm mạnh: Expectation DSL phong phú, interactive data docs, cộng đồng lớn. Điểm yếu: chính thức không hỗ trợ streaming (Issue \#7337 trên GitHub), không DC, không watermark. Khác biệt WAVES: native streaming, watermark semantics, retraction mechanism.

\medskip\noindent\textbf{Soda Core} \cite{sodacore}: Thư viện Python mã nguồn mở. Điểm mạnh: 25+ metrics, kiểm tra dựa trên YAML, hỗ trợ 18+ nguồn dữ liệu. Điểm yếu: batch-only, không streaming, không DC. Khác biệt WAVES: stream-first, DC engine với KD-tree, watermark-aware alerting.

\medskip\noindent\textbf{Spark Structured Streaming} \cite{sparkstreaming}: Phần mở rộng của Apache Spark trên Databricks. Điểm mạnh: có watermark và allowed lateness, native streaming, tích hợp sẵn trong hệ sinh thái Spark. Điểm yếu: không DC checking, chỉ cung cấp các metric cơ bản. Khác biệt WAVES: DC engine chuyên biệt với elastic box + EMA, shared rule optimizer.

\medskip\noindent\textbf{AWS Deequ / Sagemaker Model Monitor} \cite{deequ}: Công cụ của Amazon. Điểm mạnh: drift detection, baseline comparison, tích hợp sâu với AWS ecosystem. Điểm yếu: batch-only, không streaming, không DC, không watermark. Khác biệt WAVES: incremental stream processing, watermark semantics.

\medskip\noindent\textbf{GCP Dataplex Data Quality} \cite{dataplex}: Dịch vụ của Google Cloud (2024-2025). Điểm mạnh: auto scans, 5 chiều quality, custom SQL rules, tích hợp BigQuery. Điểm yếu: BigQuery table scans, không streaming, không DC. Khác biệt WAVES: stream-first, pane-based indexing, DC engine.

\medskip\noindent\textbf{Monte Carlo / Bigeye / Acceldata}: Các công cụ DQ thương mại. Điểm mạnh: auto anomaly detection, lineage tracking, observability toàn diện. Điểm yếu: giám sát ở mức bảng, không DC, không watermark. Khác biệt WAVES: row-level DC violation, event-time semantics.

\textbf{Kết luận thị trường:} Không có công cụ nào trong số này đồng thời hỗ trợ đủ bốn chiều: stream-first, denial constraints, watermark semantics và retraction. Đây là khoảng trống mà WAVES hướng tới lấp đầy.
...
\textbf{GAP 1 -- Denial Constraints trong Streaming DQ.}

Các nghiên cứu gần đây cho thấy việc phát hiện vi phạm denial constraints trên dữ liệu luồng đối mặt với bốn thách thức cốt lõi. Thứ nhất, tính đúng đắn: cần event-time semantics để phân biệt vi phạm thật với hiện tượng thiếu dữ liệu tạm thời. Thứ hai, hiệu suất: cách đối chiếu cặp bản ghi brute-force có độ phức tạp $O(N^2)$, không scale khi kích thước cửa sổ tăng. Thứ ba, quản lý trạng thái: hệ thống phải cập nhật gia tăng khi cửa sổ trượt thay vì tính lại toàn bộ. Thứ tư, retraction: dữ liệu đến muộn có thể vô hiệu hóa cảnh báo đã phát trước đó, nên cần cơ chế rút lại.

Nguồn: VLDB 2025 Martin et al. (đến 95\% constraints được suy luận là FALSE do thiếu ngữ cảnh), Incremental DC Discovery \cite{springer2024incremental}, Stream DaQ \cite{arxiv2506streamdaq}.

\textbf{GAP 2 -- Watermark và Late Data trong thực tế.}

Trong môi trường vận hành thực, watermark thường bị lag so với processing time do ba nguyên nhân chính từ Klink \cite{sigmod2021klink}: stragglers giữ watermark toàn cục, độ trễ mạng biến thiên, và các partition rỗng không produce dữ liệu. Hệ quả trực tiếp là cửa sổ không đóng được, trạng thái tiếp tục phình, và detection latency tăng.

Rapidash và Weever chưa gắn trực tiếp với event-time semantics và watermark. Các watermark papers lý thuyết tốt nhưng chưa tích hợp với engine phát hiện vi phạm. Đây là lý do WAVES đặt watermark làm thành phần trung tâm, không phải chi tiết triển khai phụ.

Nguồn: The Dataflow Model \cite{vldb2015dataflow}, Watermarks in Stream Processing \cite{vldb2017watermarks}, ProbSlack \cite{pods2018probslack}.

\textbf{GAP 3 -- Box Dropping hiệu quả trong không gian đa chiều.}

Rapidash giải bài toán phát hiện vi phạm trên dữ liệu tĩnh bằng orthogonal range search và hash partition. Tuy nhiên, khi số chiều $k$ tăng, hypercube volume tăng theo $k$, bounding boxes giao với hầu hết query regions, và box dropping trở nên kém hiệu quả --- trở về gần $O(N)$ trong không gian cao chiều.

Rapidash giải quyết bằng: tách equality columns (hash partition) khỏi inequality columns (range search), giới hạn số chiều range search bằng $k_{\max}$, và boolean range search thay vì list all matches. WAVES mở rộng bằng Shared Rule Optimizer: gom nhóm các luật có equality columns chung, giữ $k_{\max} = 5$ (configurable), và infinite padding cho các chiều không dùng.

Nguồn: Rapidash \cite{vldb2024rapidash}.

\textbf{Tổng hợp:} WAVES được định vị như kiến trúc kết nối: lấy tổ chức luồng làm vỏ, lấy phát hiện vi phạm logic làm lõi, và dùng watermark để làm lớp ra quyết định cảnh báo.
